...
@section('content')
<div class="mb-4">
    <h2 class="font-bold text-xl mb-4">Input Retur Pembelian (Ke Distributor)</h2>
</div>
...
<div class="bg-white p-6 rounded shadow">
    <form action="{{ route('owner.retur-pembelian.store') }}" method="POST">
        ...
        <div class="grid grid-cols-3 gap-4 mb-4">
            <div>
                <label class="block text-gray-700 text-sm font-bold mb-2">Distributor</label>
                <select name="id_distributor" class="w-full border rounded p-2" required>
            ...
            <div>
                <label class="block text-gray-700 text-sm font-bold mb-2">Gudang (Sumber Barang)</label>
                <select name="id_gudang" class="w-full border rounded p-2" required>
        ...
        <table class="w-full border mb-4" id="itemTable">
            <thead class="bg-gray-100">
                <tr>
                    ...
                    <th class="p-2 border text-right">Harga Beli (Saat Retur)</th>
        ...
        <div class="flex items-center justify-between mt-4 border-t pt-4">
            ...
            <a href="{{ route('owner.retur-pembelian.index') }}" class="text-blue-500 hover:text-blue-800">Batal</a>
        </div>
    </form>
</div>
...
<script>
    ...
    function addRow() {
        ...
        produks.forEach(p => {
            produkOptions += `<option value="${p.id_produk}" data-harga="${p.harga_beli}">${p.nama_produk} (${p.sku})</option>`;
        });

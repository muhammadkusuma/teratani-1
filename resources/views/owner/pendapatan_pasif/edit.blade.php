...
<div class="bg-white border border-gray-400 p-4">
    <form action="{{ route("owner.pendapatan_pasif.update", $pendapatanPasif->id_pendapatan_pasif)') }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method("PUT")
        <div class="grid grid-cols-2 gap-4">
            <div><label class="block text-xs font-bold("kode_pendapatan_pasif", $pendapatanPasif->kode_pendapatan_pasif)) }}" required class="w-full border border-gray-400 p-1 text-xs shadow-inner font-mono"></div>
            <div><label class="block text-xs font-bold("tanggal_pendapatan_pasif", $pendapatanPasif->tanggal_pendapatanPasif->format("Y-m-d"))()->format('Y-m-d')) }}" required class="w-full border border-gray-400 p-1 text-xs shadow-inner"></div>
        </div>
        <div class="grid grid-cols-2 gap-4 mt-3">
            ...
            <div><label class="block text-xs font-bold mb-1">Jumlah (Rp) <span class="text-green-600">*</span></label><input type="number" name="jumlah" value="{{ old('jumlah', $pendapatanPasif->jumlah) }}" required min="0" step="1000" class="w-full border border-gray-400 p-1 text-xs shadow-inner"></div>
        </div>

...
<div class="bg-white border border-gray-400 p-4">
    <div class="grid grid-cols-2 gap-4">
        <div>
            ...
            <div class="space-y-2 text-xs">
                ...
                <div class="flex"><span class="w-32 text-gray-600">Kategori:</span><span class="bg-purple-100 text-purple-800 px-2 py-0.5 rounded font-bold">{{ $pendapatan_pasif->kategori }}</span></div>
                <div class="flex"><span class="w-32 text-gray-600">Jumlah:</span><span class="font-bold text-green-700 text-lg">Rp {{ number_format($pendapatan_pasif->jumlah, 0, ',', '.') }}</span></div>
                <div class="flex"><span class="w-32 text-gray-600">Metode Bayar:</span><span class="bg-blue-100 text-blue-800 px-2 py-0.5 rounded font-bold">{{ $pendapatan_pasif->metode_terima }}</span></div>
                <div class="flex"><span class="w-32 text-gray-600">Dicatat oleh:</span><span class="font-semibold">{{ $pendapatan_pasif->user->name ?? '-' }}</span></div>
            </div>
        </div>
        <div>
            ...
            <div class="text-xs space-y-3">
                <div>
                    ...
                    <div class="bg-gray-50 border border-gray-300 p-2 rounded">{{ $pendapatan_pasif->sumber }}</div>
                </div>
                @if($pendapatan_pasif->keterangan)
                <div>
                    <div class="text-gray-600 mb-1">Keterangan:</div>
                    <div class="bg-gray-50 border border-gray-300 p-2 rounded">{{ $pendapatan_pasif->keterangan }}</div>
                </div>
                @endif
            </div>
        </div>
    </div>
    @if($pendapatan_pasif->bukti_penerimaan)
    <div class="mt-4 border-t border-gray-300 pt-4">
        ...
        <div class="bg-gray-50 border border-gray-300 p-3 rounded">
            @if(Str::endsWith($pendapatan_pasif->bukti_penerimaan, '.pdf'))
                <a href="{{ asset('storage/' . $pendapatan_pasif->bukti_penerimaan) }}" target="_blank" class="text-blue-600 hover:underline text-xs">
                    <i class="fa fa-file-pdf text-green-500 text-2xl"></i> Lihat Bukti Penerimaan (PDF)
                </a>
            @else
                <img src="{{ asset('storage/' . $pendapatan_pasif->bukti_penerimaan) }}" alt="Bukti Penerimaan" class="max-w-md border border-gray-400">
            @endif
        </div>
    </div>
    @endif

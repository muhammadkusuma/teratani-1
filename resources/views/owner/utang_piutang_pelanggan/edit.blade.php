...
<div class="mb-3">
    <h2 class="font-bold text-lg border-b-2 border-gray-500 inline-block pr-4">
        <i class="fa fa-edit"></i> EDIT TRANSAKSI PIUTANG PELANGGAN
    </h2>
</div>
...
<div class="bg-white border border-gray-400 p-4">
    <form action="{{ route('owner.utang-piutang-pelanggan.update', $transaksi->id_piutang) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <select name="jenis_transaksi" required class="w-full border border-gray-400 p-2 text-xs shadow-inner @error('jenis_transaksi') border-red-500 @enderror">
                    ...
                    <option value="piutang" {{ old('jenis_transaksi', $transaksi->jenis_transaksi) == 'piutang' ? 'selected' : '' }}>Piutang (Tambah Piutang)</option>
            ...
            <div>
                ...
                <input type="text" name="no_referensi" value="{{ old('no_referensi', $transaksi->no_referensi) }}" maxlength="50" placeholder="No Invoice, Nota, dll" class="w-full border border-gray-400 p-2 text-xs shadow-inner @error('no_referensi') border-red-500 @enderror">
                @error('no_referensi')
                    <span class="text-red-600 text-[10px]">{{ $message }}</span>
                @enderror
            </div>
        ...
        <div class="flex gap-2 mt-4">
            ...
            <a href="{{ route('owner.utang-piutang-pelanggan.index') }}" class="px-4 py-2 bg-gray-400 text-white border border-gray-600 text-xs hover:bg-gray-300">

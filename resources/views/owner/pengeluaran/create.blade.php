...
<div class="flex justify-between items-center mb-3">
    <h2 class="font-bold text-lg border-b-2 border-gray-500 pr-4"><i class="fa fa-plus-circle"></i> TAMBAH PENGELUARAN BARU</h2>
...
<div class="bg-white border border-gray-400 p-4">
    <form action="{{ route('owner.pengeluaran.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><label class="block text-xs font-bold mb-1">Kode Pengeluaran <span class="text-red-600">*</span></label><input type="text" name="kode_pengeluaran" value="{{ old('kode_pengeluaran', $kodePengeluaran) }}" required class="w-full border border-gray-400 p-1 text-xs shadow-inner font-mono"></div>
            <div><label class="block text-xs font-bold mb-1">Tanggal <span class="text-red-600">*</span></label><input type="date" name="tanggal_pengeluaran" value="{{ old('tanggal_pengeluaran', now()->format('Y-m-d')) }}" required class="w-full border border-gray-400 p-1 text-xs shadow-inner"></div>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-3">
            ...
            <div><label class="block text-xs font-bold mb-1">Jumlah (Rp) <span class="text-red-600">*</span></label><input type="number" name="jumlah" value="{{ old('jumlah', 0) }}" required min="0" step="1000" class="w-full border border-gray-400 p-1 text-xs shadow-inner"></div>
        </div>
        <div class="mt-3"><label class="block text-xs font-bold mb-1">Deskripsi <span class="text-red-600">*</span></label><textarea name="deskripsi" rows="3" required class="w-full border border-gray-400 p-1 text-xs shadow-inner">{{ old('deskripsi') }}</textarea></div>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-3">
        ...
        <div class="flex gap-2 mt-4">
            <button type="submit" class="bg-blue-700 text-white border border-blue-900 px-4 py-2 text-xs hover:bg-blue-600"><i class="fa fa-save"></i> SIMPAN PENGELUARAN</button>

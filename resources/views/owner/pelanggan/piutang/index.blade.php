...
<div class="flex justify-between items-center mb-3">
    ...
    <a href="{{ route('owner.pelanggan.piutang.create') }}" class="px-3 py-1 bg-blue-700 text-white border border-blue-900 shadow hover:bg-blue-600 text-xs">
...
<div class="bg-white border border-gray-400 p-3 mb-3">
    <form method="GET" action="{{ route('owner.pelanggan.piutang.index') }}" class="grid grid-cols-5 gap-3">
        <div>
        ...
        <div class="flex items-end gap-2">
            ...
            <a href="{{ route('owner.pelanggan.piutang.index') }}" class="bg-gray-400 text-white border border-gray-600 px-4 py-1 text-xs hover:bg-gray-300">
            ...
            <tr class="hover:bg-yellow-50 text-xs">
                ...
                <td class="border border-gray-300 p-2 text-center">
                    <div class="flex justify-center gap-1">
                        <a href="{{ route('owner.pelanggan.piutang.edit', $row->id_piutang) }}" class="bg-yellow-400 border border-yellow-600 px-2 py-0.5 text-[10px] hover:bg-yellow-300">EDIT</a>
                        <form action="{{ route('owner.pelanggan.piutang.destroy', $row->id_piutang) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus transaksi ini? Saldo akan di-recalculate.')">
                            @csrf
                            @method('DELETE')

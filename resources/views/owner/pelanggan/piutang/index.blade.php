...
<div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-4">
    <h2 class="font-bold text-xl border-b-4 border-blue-600 pb-1 pr-6 uppercase tracking-tight">
        <i class="fa fa-hand-holding-usd text-blue-700"></i> Piutang Pelanggan
    </h2>
    <a href="{{ route('owner.pelanggan.piutang.create') }}" class="w-full md:w-auto text-center px-4 py-2 bg-blue-700 text-white border border-blue-900 shadow-md hover:bg-blue-600 text-xs font-bold transition-all rounded-sm uppercase tracking-wider">
...
@if(session('error'))
    <div class="bg-rose-50 border-l-4 border-rose-500 text-rose-800 p-3 mb-4 shadow-sm text-xs flex items-center gap-2">
        <i class="fa fa-exclamation-circle text-rose-600 text-sm"></i> 
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
    <div class="bg-gradient-to-br from-blue-50 to-white border-l-4 border-blue-600 p-4 shadow-sm rounded-r-md">
        <div class="text-[10px] text-blue-800 font-black uppercase tracking-widest mb-1">Total Piutang (Terfilter)</div>
        <div class="text-2xl font-black text-blue-900 leading-none">Rp {{ number_format($summary['total_piutang'], 0, ',', '.') }}</div>
    </div>
    ...
    <div class="bg-gradient-to-br from-amber-50 to-white border-l-4 border-amber-600 p-4 shadow-sm rounded-r-md">
        <div class="text-[10px] text-amber-800 font-black uppercase tracking-widest mb-1">Saldo Piutang (Belum Bayar)</div>
...
<div class="bg-white border border-gray-300 p-4 mb-4 shadow-sm rounded-sm">
    <form method="GET" action="{{ route('owner.pelanggan.piutang.index') }}" class="grid grid-cols-1 md:grid-cols-5 gap-3">
        <div class="md:col-span-1">
            <label class="block text-[10px] font-black text-gray-500 uppercase mb-1 tracking-wider">Pelanggan</label>
            <select name="id_pelanggan" class="w-full border border-gray-300 p-1.5 text-xs shadow-inner focus:border-blue-500 outline-none transition-all bg-gray-50">
        ...
        <div class="flex items-end gap-2">
            <button type="submit" class="flex-1 bg-blue-600 text-white border border-blue-800 px-4 py-1.5 text-xs font-bold hover:bg-blue-500 transition-colors shadow-sm uppercase tracking-wider">
                <i class="fa fa-filter"></i> Filter
            </button>
            <a href="{{ route('owner.pelanggan.piutang.index') }}" class="bg-gray-100 text-gray-700 border border-gray-300 px-4 py-1.5 text-xs font-bold hover:bg-gray-200 transition-colors text-center shadow-sm uppercase tracking-wider">
...
<div class="block md:hidden space-y-4 mb-4">
    ...
    <div class="bg-white border-t-4 {{ $row->jenis_transaksi == 'piutang' ? 'border-blue-500' : 'border-emerald-500' }} p-4 shadow-lg rounded-sm relative active:scale-[0.98] transition-all">
        <div class="flex justify-between items-start mb-2">
            <div>
                <div class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">{{ $row->tanggal->format('d M Y') }}</div>
                <h3 class="font-black text-sm text-blue-900 tracking-tight leading-tight">{{ $row->pelanggan->nama_pelanggan }}</h3>
                @if($row->no_referensi)
            ...
            <div>
                @if($row->jenis_transaksi == 'piutang')
                    <span class="bg-blue-100 text-blue-800 border border-blue-200 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-tighter shadow-sm">Piutang</span>
                @else
        ...
        <div class="grid grid-cols-2 gap-3 mb-3 border-y border-gray-100 py-3">
            <div>
                ...
                <div class="text-xs font-black {{ $row->jenis_transaksi == 'piutang' ? 'text-blue-600' : 'text-emerald-600' }}">
                    {{ $row->jenis_transaksi == 'piutang' ? '+' : '-' }} Rp {{ number_format($row->nominal, 0, ',', '.') }}
                </div>
            </div>
            <div>
                <div class="text-[9px] text-gray-400 font-bold uppercase tracking-tight font-black">Sisa Tagihan</div>
        ...
        <div class="grid grid-cols-2 gap-2">
            <a href="{{ route('owner.pelanggan.piutang.edit', $row->id_piutang) }}" class="bg-amber-400 border border-amber-600 text-amber-900 py-2 px-1 text-center text-[10px] font-black hover:bg-amber-300 transition-colors rounded-sm shadow-sm uppercase tracking-tighter">Edit</a>
            <form action="{{ route('owner.pelanggan.piutang.destroy', $row->id_piutang) }}" method="POST" class="w-full" onsubmit="return confirm('Hapus transaksi ini? Saldo akan dihitung ulang secara otomatis.')">
                @csrf
                @method('DELETE')
    ...
    <table class="w-full text-left border-collapse">
        <thead>
            <tr class="bg-blue-900 text-white text-[10px] font-black uppercase tracking-widest">
                ...
                <th class="border border-blue-900 p-3 text-right bg-emerald-400/20 text-blue-900">Saldo Piutang</th>
        ...
        <tbody>
            ...
            <tr class="hover:bg-blue-50 transition-colors text-xs border-b border-gray-200">
                ...
                <td class="p-3">
                    <div class="font-black text-blue-900 uppercase tracking-tight leading-tight">{{ $row->pelanggan->nama_pelanggan }}</div>
                    @if($row->no_referensi)
                ...
                <td class="p-3 text-center">
                    @if($row->jenis_transaksi == 'piutang')
                        <span class="px-2 py-0.5 rounded-full bg-blue-100 text-blue-800 text-[9px] font-black uppercase tracking-tighter border border-blue-200">Piutang</span>
                    @else
                ...
                <td class="p-3 text-right font-black">
                    <span class="{{ $row->jenis_transaksi == 'piutang' ? 'text-blue-600' : 'text-emerald-700' }}">
                        {{ $row->jenis_transaksi == 'piutang' ? '+' : '-' }} Rp {{ number_format($row->nominal, 0, ',', '.') }}
                    </span>
                </td>
                <td class="p-3 text-right font-black bg-emerald-50/30 text-gray-900 border-x border-gray-100">
                    Rp {{ number_format($row->saldo_piutang, 0, ',', '.') }}
                </td>
                <td class="p-3 text-gray-500 italic text-[10px] leading-snug">{{ Str::limit($row->keterangan ?? '-', 50) }}</td>
                <td class="p-3">
                    <div class="flex justify-center gap-1">
                        <a href="{{ route('owner.pelanggan.piutang.edit', $row->id_piutang) }}" class="bg-amber-400 border border-amber-500 text-amber-900 px-3 py-1 text-[10px] font-black hover:bg-amber-300 transition-colors shadow-sm rounded-sm uppercase tracking-tighter">Edit</a>
                        <form action="{{ route('owner.pelanggan.piutang.destroy', $row->id_piutang) }}" method="POST" class="inline" onsubmit="return confirm('Hapus transaksi ini?')">
                            @csrf
                            @method('DELETE')

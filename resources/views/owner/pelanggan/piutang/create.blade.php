...
<div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-4">
    <h2 class="font-bold text-lg border-b-4 border-blue-600 pb-1 pr-6 uppercase tracking-tight">
        <i class="fa fa-plus-circle text-blue-700"></i> Tambah Transaksi Piutang
    </h2>
    ...
    <form action="{{ route('owner.pelanggan.piutang.store') }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
                ...
                <select name="jenis_transaksi" required class="w-full border p-2 text-xs shadow-inner bg-gray-50 focus:bg-white focus:border-blue-500 transition-all outline-none @error('jenis_transaksi') border-rose-500 @else border-gray-300 @enderror">
                    ...
                    <option value="piutang" {{ old('jenis_transaksi') == 'piutang' ? 'selected' : '' }}>Piutang (Tambah Tagihan)</option>
            ...
            <div>
                ...
                <input type="text" name="no_referensi" value="{{ old('no_referensi') }}" maxlength="50" placeholder="No Invoice, Nota, dll" class="w-full border p-2 text-xs shadow-inner focus:border-blue-500 transition-all outline-none @error('no_referensi') border-rose-500 @else border-gray-300 @enderror">
                @error('no_referensi')
                    <span class="text-rose-600 text-[10px] font-bold mt-1 block uppercase">{{ $message }}</span>
                @enderror
            </div>
        ...
        <div class="bg-amber-50 border-l-4 border-amber-400 p-3 mt-5 shadow-sm">
            ...
            <p class="text-xs text-amber-800 font-medium leading-normal italic">
                Saldo piutang pelanggan akan dihitung dan diurutkan kembali secara otomatis berdasarkan tanggal transaksi setelah data disimpan.
            </p>
        </div>
